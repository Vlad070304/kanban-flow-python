...
class AnalyticsWindow(tk.Toplevel):
    ...
    def _render_native_chart(
        self, parent_frame: tk.Widget, metrics: dict[str, Any]
    ) -> None:
        ...
        for i in range(len(line_points) - 1):
            pt1 = line_points[i]
            pt2 = line_points[i + 1]
            canvas.create_line(pt1[0], pt1[1], pt2[0], pt2[1], fill="#FAB387", width=2)

        for pt in line_points:
            canvas.create_oval(
                pt[0] - 3, pt[1] - 3, pt[0] + 3, pt[1] + 3, fill="#FAB387", outline=""
            )

    def _create_stat_card(
        self, parent: tk.Widget, title: str, value: str, accent_color: str
    ) -> None:
